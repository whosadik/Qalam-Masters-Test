          ...
          <div className="flex items-center justify-between py-3 md:py-4">
            ...
            {!isAuthenticated ? (
              <nav className="hidden md:flex items-center gap-8">
                <Link to="/about-journal" className="text-gray-600 hover:text-gray-900 transition-colors">О платформе</Link>
                <Link to="/editorial-board" className="text-gray-600 hover:text-gray-900 transition-colors">Редколлегия</Link>
                <Link to="/author-info" className="text-gray-600 hover:text-gray-900 transition-colors">Информация для авторов</Link>
                <Link to="/publication-terms" className="text-gray-600 hover:text-gray-900 transition-colors">Условия публикации</Link>
              </nav>
            ) : (
              <nav className="hidden md:flex items-center gap-8">
            ...
            <div className="hidden md:flex items-center gap-3">
              ...
              {!isAuthenticated ? (
                <>
                  ...
                  <Button variant="ghost" onClick={demoLogin} title="Демо-вход">Демо-вход</Button>
                </>
              ) : (
                <>
                  ...
                  <NotificationsButton
                    count={3}
                    items={[
                      { id: 1, title: "Новая статья на рецензию", time: "2 минуты назад" },
                      { id: 2, title: "Рецензия готова", time: "1 час назад" },
                      { id: 3, title: "Статья принята к публикации", time: "3 часа назад" },
                    ]}
                  />
                  <ProfileMenu
            ...
            <div className="md:hidden flex items-center gap-1">
              ...
              {!isAuthenticated ? (
                <>
                  <Link to="/login"><Button variant="outline" size="sm">Войти</Button></Link>
                  <Link to="/register"><Button size="sm">Регистрация</Button></Link>
                  <Button variant="ghost" size="sm" onClick={demoLogin} title="Демо-вход">Демо</Button>
                </>
              ) : (
                <>
                  <Link to="/author-dashboard"><Button variant="outline" size="sm">ЛК</Button></Link>
                  <NotificationsButton
                    count={3}
                    items={[
                      { id: 1, title: "Новая статья на рецензию", time: "2 минуты назад" },
                  ...
                  <ProfileMenu
                    ...
                    onProfile={() => { closeMenu(); navigate("/author-profile"); }}
                    onSettings={() => { closeMenu(); navigate("/settings"); }}
                    onLogout={() => { logout(); closeMenu(); navigate("/"); }}
                  />
                </>
              )}
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-6 pt-20">
          {!isAuthenticated ? (
            <nav className="grid gap-4">
              ...
              <Link to="/editorial-board" onClick={closeMenu} className="py-2 text-base text-gray-700 hover:text-gray-900">Редколлегия</Link>
              <Link to="/author-info" onClick={closeMenu} className="py-2 text-base text-gray-700 hover:text-gray-900">Информация для авторов</Link>
              <Link to="/publication-terms" onClick={closeMenu} className="py-2 text-base text-gray-700 hover:text-gray-900">Условия публикации</Link>
            </nav>
          ) : (
            <nav className="grid gap-4">
              <Link to="/author-dashboard" onClick={closeMenu} className="py-2 text-base text-gray-700 hover:text-gray-900">Мои статьи</Link>
              <Link to="/reviews" onClick={closeMenu} className="py-2 text-base text-gray-700 hover:text-gray-900">Рецензии</Link>
              <Link to="/messages" onClick={closeMenu} className="py-2 text-base text-gray-700 hover:text-gray-900">Переписка</Link>
              <Link to="/archive" onClick={closeMenu} className="py-2 text-base text-gray-700 hover:text-gray-900">Архив</Link>
            </nav>
          )}
        </div>
      </div>

      <DashboardNavigatorModal open={navOpen} onClose={() => setNavOpen(false)} items={filteredNavItems} />
    </>
  );
}
